...
// possible to walk much slower than this
#define NUM_AUTOCORR_LAGS 50

// length of derivative filter
#define DERIV_FILT_LEN 5

// length of FIR low pass filter
#define LPF_FILT_LEN 9

// this is the min delta between peak and trough of autocorrelation peak
#define AUTOCORR_DELTA_AMPLITUDE_THRESH 5e8

// this is the min number of points the autocorrelation peak should be on either
// side of the peak
#define AUTOCORR_MIN_HALF_LEN 3

static int8_t deriv_coeffs[DERIV_FILT_LEN] = {
    -6, 31, 0, -31, 6};  // coefficients of derivative filter from
// https://www.dsprelated.com/showarticle/814.php
static int8_t lpf_coeffs[LPF_FILT_LEN] = {
    -5, 6,  34, 68, 84,
    68, 34, 6,  -5};  // coefficients of FIR low pass filter generated in matlab
// using FDATOOL
static uint8_t mag_sqrt[NUM_TUPLES] = {
    0};  // this holds the square root of magnitude data of X,Y,Z (so its length
// is NUM_SAMPLES/3)
static int32_t lpf[NUM_TUPLES] = {0};  // hold the low pass filtered signal
static int64_t autocorr_buff[NUM_AUTOCORR_LAGS] = {
    0};  // holds the autocorrelation results
...
static void get_autocorr_peak_stats(int64_t* autocorr_buff,
                                    uint8_t* neg_slope_count,
                                    int64_t* delta_amplitude_right,
                                    uint8_t* pos_slope_count,
                                    int64_t* delta_amplitude_left,
                                    uint8_t peak_ind);
...
// the number of points to the right and left of the peak, as well as the delta
// amplitude
static void get_autocorr_peak_stats(int64_t* autocorr_buff,
                                    uint8_t* neg_slope_count,
                                    int64_t* delta_amplitude_right,
                                    ...
                                    int64_t* delta_amplitude_left,
                                    uint8_t peak_ind) {
  // first look to the right of the peak. walk forward until the slope begins
  // decreasing
  uint8_t neg_slope_ind = peak_ind;
  uint16_t loop_limit = NUM_AUTOCORR_LAGS - 1;
  while (
      (autocorr_buff[neg_slope_ind + 1] - autocorr_buff[neg_slope_ind] < 0) &&
      (neg_slope_ind < loop_limit)) {
    *neg_slope_count = *neg_slope_count + 1;
    neg_slope_ind = neg_slope_ind + 1;
  }

  // get the delta amplitude between peak and right trough
  *delta_amplitude_right =
      autocorr_buff[peak_ind] - autocorr_buff[neg_slope_ind];

  // next look to the left of the peak. walk backward until the slope begins
  // increasing
  uint8_t pos_slope_ind = peak_ind;
  loop_limit = 0;
  while (
      (autocorr_buff[pos_slope_ind] - autocorr_buff[pos_slope_ind - 1] > 0) &&
      (pos_slope_ind > loop_limit)) {
  ...
  *delta_amplitude_left =
      autocorr_buff[peak_ind] - autocorr_buff[pos_slope_ind];
}
...
static uint8_t get_precise_peakind(int64_t* autocorr_buff, uint8_t peak_ind) {
  ...
  if ((autocorr_buff[peak_ind] > autocorr_buff[peak_ind - 1]) &&
      (autocorr_buff[peak_ind] > autocorr_buff[peak_ind + 1])) {
    // peak_ind is perfectly set at the peak. nothing to do
  } else if ((autocorr_buff[peak_ind] > autocorr_buff[peak_ind + 1]) &&
             (autocorr_buff[peak_ind] < autocorr_buff[peak_ind - 1])) {
    // peak is to the left. keep moving in that direction
    loop_limit = 0;
    while ((autocorr_buff[peak_ind] > autocorr_buff[peak_ind + 1]) &&
           (autocorr_buff[peak_ind] < autocorr_buff[peak_ind - 1]) &&
           (loop_limit < 10)) {
      peak_ind = peak_ind - 1;
      loop_limit++;
...
// this takes a lot of computation. there are efficient implementations, but
...
uint8_t count_steps(int8_t* data) {
  ...
  for (i = 0; i < NUM_TUPLES; i++) {
    temp_mag =
        (uint16_t)((uint16_t)data[i * 3 + 0] * (uint16_t)data[i * 3 + 0] +
  ...
  // look for first zero crossing where derivative goes from positive to
  // negative. that corresponds to the first positive peak in the
  // autocorrelation. look at two samples instead of just one to maybe reduce
  // the chances of getting tricked by noise
  uint8_t peak_ind = 0;
  // start index is set to 8 lags, which corresponds to a walking rate of 2.5Hz
  // @20Hz sampling rate. its probably running if its faster than this
  for (i = 8; i < NUM_AUTOCORR_LAGS; i++) {
    if ((deriv[i] > 0) && (deriv[i - 1] > 0) && (deriv[i - 2] < 0) &&
        (deriv[i - 3] < 0)) {
      peak_ind = i - 1;
      break;
    }
  ...
  get_autocorr_peak_stats(autocorr_buff, &neg_slope_count,
                          &delta_amplitude_right, &pos_slope_count,
                          &delta_amplitude_left, peak_ind);

  // now check the conditions to see if it was a real peak or not, and if so,
  // calculate the number of steps
  uint8_t num_steps = 0;
  if ((pos_slope_count > AUTOCORR_MIN_HALF_LEN) &&
      (neg_slope_count > AUTOCORR_MIN_HALF_LEN) &&
      (delta_amplitude_right > AUTOCORR_DELTA_AMPLITUDE_THRESH) &&
      (delta_amplitude_left > AUTOCORR_DELTA_AMPLITUDE_THRESH)) {
    // the period is peak_ind/sampling_rate seconds. that corresponds to a
    // frequency of 1/period with the frequency known, and the number of seconds
    // is 4 seconds, you can then find out the number of steps

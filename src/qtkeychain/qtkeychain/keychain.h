...
enum Error {
  ...
  OtherError /**< Something else went wrong (errorString() might provide
                details) */
};
...
class QKEYCHAIN_EXPORT Job : public QObject {
 ...
 public:
  ...
  /**
   * @return The QSettings instance used as plaintext storage if
  ...
  /**
   * @return Set the QSettings instance that will be used as plaintext storage
   * if insecureFallback() is true.
   * @see settings()
   * @see insecureFallback()
   */
  ...
  /**
   * @return An error message that might provide details if error() returns
   * OtherError.
   */
  QString errorString() const;

  /**
   * @return Whether this job autodeletes itself once finished() has been
  ...
  /**
   * Set whether this job should autodelete itself once finished() has been
   * emitted.
   * @see autoDelete()
   */
  ...
  /**
   * @return Whether this job will use plaintext storage on unsupported
  ...
  /**
   * Set the @p key that this job will use to read or write data from/to the
   * keychain. The key can be an empty string.
   * @see key()
   */
 ...
 * You can use a ReadPasswordJob to read passwords or binary data from the
 * keychain. This job requires a "service" string, which is basically a
 * namespace of keys within the keychain. This means that you can read all the
 * pairs <key, secret> stored in the same service string.
 */
 ...
 * You can use a WritePasswordJob to store passwords or binary data in the
 * keychain. This job requires a "service" string, which is basically a
 * namespace of keys within the keychain. This means that you can store
 * different pairs <key, secret> under the same service string.
 */
 ...
 * You can use a DeletePasswordJob to delete passwords or binary data from the
 * keychain. This job requires a "service" string, which is basically a
 * namespace of keys within the keychain. This means that you can delete all the
 * pairs <key, secret> stored in the same service string.
 */
 ...
 *
 * Note that using the insecure fallback will work even if no secure backend is
 * available.
 *
 * @since 0.14.0
 */

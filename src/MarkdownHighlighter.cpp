...
void MarkdownHighlighter::initHighlightingRules() {
  ...
  // highlight block quotes
  {
    ...
    rule.pattern =
        QRegularExpression(_highlightingOptions.testFlag(
  ...
  // highlight urls
  {
    ...
    rule.pattern =
        QRegularExpression(QStringLiteral(R"(\[([^\[\]]+)\]\((\S+|.+?)\)\B)"));
    rule.shouldContain = QStringLiteral("](");
    _highlightingRules.append(rule);
...
void MarkdownHighlighter::highlightHeadline(const QString &text) {
  ...
  if (headingFound) {
    ...
    if (headingLevel > 0) {
      ...
      // Set styling of the "#"s to "masked syntax", but with the size of the
      // heading
...
void MarkdownHighlighter::highlightCodeBlock(const QString &text,
  ...
  if (text.startsWith(opener)) {
    ...
    if ((previousBlockState() != CodeBlock &&
      ...
      const int state =
          opener == QLatin1String("```") ? CodeBlockEnd : CodeBlockTildeEnd;
      setCurrentBlockState(state);
    }
...
void MarkdownHighlighter::taggerScriptHighlighter(const QString &text) {
  ...
  for (int i = 0; i < textLen; ++i) {
    ...
    if (text.at(i) == QChar('$') && MH_SUBSTR(i, 5) != QLatin1String("$noop")) {
...
int MarkdownHighlighter::highlightInlineSpans(const QString &text,
  ...
  if (fmt.fontPointSize() > 0) inlineFmt.setFontPointSize(fmt.fontPointSize());
    ...
    while (j >= 0) {
      ...
      if (curDelim.open && curDelim.marker == lastDelim.marker &&
        ...
        const bool oddMatch = (curDelim.close || lastDelim.open) &&
                              curDelim.len != -1 && lastDelim.len != -1 &&
  ...
  for (int i = delims.length() - 1; i >= 0; --i) {
    ...
    if (isStrong) {
      ...
      _ranges[block].append(
          InlineRange(startDelim.pos, endDelim.pos + 1, RangeType::Emphasis));
      _ranges[block].append(
          InlineRange(startDelim.pos - 1, endDelim.pos, RangeType::Emphasis));
      --i;
    } else {

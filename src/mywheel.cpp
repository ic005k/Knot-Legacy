...
#include "src/qwt/qwt.h"
#include "src/qwt/qwt_math.h"
#include "src/qwt/qwt_painter.h"
#include "src/qwt/qwt_wheel.h"
...
//! Destructor
QwtWheel::~QwtWheel() { delete m_data; }
...
/*!
   ...
   \sa isTracking()
 */
void QwtWheel::setTracking(bool enable) { m_data->tracking = enable; }

/*!
   \return True, when tracking is enabled
   \sa setTracking(), valueChanged(), wheelMoved()
 */
bool QwtWheel::isTracking() const { return m_data->tracking; }
 ...
 */
int QwtWheel::updateInterval() const { return m_data->updateInterval; }
...
void QwtWheel::mouseMoveEvent(QMouseEvent* event) {
  ...
  if (m_data->stepAlignment) value = alignedValue(value);
...
void QwtWheel::mouseReleaseEvent(QMouseEvent* event) {
  ...
  if (m_data->mass > 0.0) {
    ...
    if ((std::fabs(m_data->speed) > 0.0) && (ms < 50)) startFlying = true;
  }
  ...
  if (startFlying) {
    ...
    if (m_data->pendingValueChanged) Q_EMIT valueChanged(m_data->value);
  }
...
void QwtWheel::keyPressEvent(QKeyEvent* event) {
  ...
  if (event->isAccepted()) stopFlying();
...
/*!
   ...
   \sa setTickCnt()
 */
int QwtWheel::tickCount() const { return m_data->tickCount; }
 ...
 */
int QwtWheel::wheelBorderWidth() const { return m_data->wheelBorderWidth; }
...
/*!
   ...
   \sa setBorderWidth()
 */
int QwtWheel::borderWidth() const { return m_data->borderWidth; }
...
/*!
   ...
   \sa setTotalAngle()
 */
double QwtWheel::totalAngle() const { return m_data->totalAngle; }
...
void QwtWheel::setOrientation(Qt::Orientation orientation) {
  if (m_data->orientation == orientation) return;
...
/*!
   ...
   \sa setOrientation()
 */
Qt::Orientation QwtWheel::orientation() const { return m_data->orientation; }
 ...
 */
double QwtWheel::viewAngle() const { return m_data->viewAngle; }
...
void QwtWheel::paintEvent(QPaintEvent* event) {
  ...
  qDrawShadePanel(&painter, contentsRect(), palette(), true,
                  m_data->borderWidth);

  drawWheelBackground(&painter, wheelRect());
  drawTicks(&painter, wheelRect());

  if (hasFocus()) QwtPainter::drawFocusRect(&painter, this);
}
...
void QwtWheel::drawWheelBackground(QPainter* painter, const QRectF& rect) {
  ...
  const QPen darkPen(pal.color(QPalette::Dark), m_data->wheelBorderWidth,
...
void QwtWheel::drawTicks(QPainter* painter, const QRectF& rect) {
  ...
  const QPen lightPen(palette().color(QPalette::Light), 0, Qt::SolidLine,
                      Qt::FlatCap);
  const QPen darkPen(palette().color(QPalette::Dark), 0, Qt::SolidLine,
                     Qt::FlatCap);
...
/*!
   ...
   \sa setWheelWidth()
 */
int QwtWheel::wheelWidth() const { return m_data->wheelWidth; }
...
QSize QwtWheel::minimumSizeHint() const {
  ...
  if (m_data->orientation != Qt::Horizontal) sz.transpose();
...
/*!
   ...
   \sa setSingleStep()
 */
double QwtWheel::singleStep() const { return m_data->singleStep; }
 ...
 */
bool QwtWheel::stepAlignment() const { return m_data->stepAlignment; }
 ...
 */
int QwtWheel::pageStepCount() const { return m_data->pageStepCount; }
...
void QwtWheel::setRange(double min, double max) {
  ...
  if (m_data->minimum == min && m_data->maximum == max) return;
...
/*!
   ...
   \note The maximum is adjusted if necessary to ensure that the range remains
   valid.
 */
void QwtWheel::setMinimum(double value) { setRange(value, maximum()); }

/*!
   \return The minimum of the range
   \sa setRange(), setMinimum(), maximum()
 */
double QwtWheel::minimum() const { return m_data->minimum; }
...
/*!
   ...
   \sa setRange(), setMinimum(), maximum()
 */
void QwtWheel::setMaximum(double value) { setRange(minimum(), value); }

/*!
   \return The maximum of the range
   \sa setRange(), setMaximum(), minimum()
 */
double QwtWheel::maximum() const { return m_data->maximum; }
 ...
 */
double QwtWheel::value() const { return m_data->value; }
...
/*!
   ...
   \sa setInverted()
 */
bool QwtWheel::isInverted() const { return m_data->inverted; }
...
/*!
   ...
   \sa wrapping()
 */
void QwtWheel::setWrapping(bool on) { m_data->wrapping = on; }

/*!
   \return True, when wrapping is set
   \sa setWrapping()
 */
bool QwtWheel::wrapping() const { return m_data->wrapping; }
...
/*!
   ...
   \sa setMass()
 */
double QwtWheel::mass() const { return m_data->mass; }

...
class Point {
 public:
  ...
  constexpr explicit Point(XYPOSITION x_ = 0, XYPOSITION y_ = 0) noexcept
      : x(x_), y(y_) {}
...
class PRectangle {
 public:
  ...
  constexpr explicit PRectangle(XYPOSITION left_ = 0, XYPOSITION top_ = 0,
                                XYPOSITION right_ = 0,
                                XYPOSITION bottom_ = 0) noexcept
      : left(left_), top(top_), right(right_), bottom(bottom_) {}

  static PRectangle FromInts(int left_, int top_, int right_,
                             int bottom_) noexcept {
    return PRectangle(
        static_cast<XYPOSITION>(left_), static_cast<XYPOSITION>(top_),
  ...
  bool ContainsWholePixel(Point pt) const noexcept {
    ...
    return (pt.x >= left) && ((pt.x + 1) <= right) && (pt.y >= top) &&
           ((pt.y + 1) <= bottom);
  }
  ...
  bool Empty() const noexcept { return (Height() <= 0) || (Width() <= 0); }
};
...
class ColourDesired {
 ...
 public:
  ...
  ColourDesired(unsigned int red, unsigned int green,
  ...
  unsigned char GetRed() const noexcept { return co & 0xff; }
  unsigned char GetGreen() const noexcept { return (co >> 8) & 0xff; }
  unsigned char GetBlue() const noexcept { return (co >> 16) & 0xff; }

  // Red, green and blue values as float 0..1.0
  float GetRedComponent() const noexcept { return GetRed() / componentMaximum; }
...
class ColourAlpha : public ColourDesired {
 public:
  ...
  ColourAlpha(unsigned int red, unsigned int green, unsigned int blue,
  ...
  unsigned char GetAlpha() const noexcept { return (AsInteger() >> 24) & 0xff; }
...
struct FontParameters {
  ...
  FontParameters(const char *faceName_, float size_ = 10, int weight_ = 400,
                 bool italic_ = false, int extraFontFlag_ = 0,
                 int technology_ = 0, int characterSet_ = 0) noexcept
      :
...
class Surface {
 public:
  ...
  virtual void GradientRectangle(PRectangle rc,
...
// ScintillaBase implements IListBoxDelegate to receive ListBoxEvents from a
// ListBox
...
class Platform {
 public:
  ...
  static void Assert(const char *c, const char *file,
                     int line) CLANG_ANALYZER_NORETURN;
};
